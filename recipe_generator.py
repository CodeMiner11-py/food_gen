...
client = genai_client_module.Client()

def generate_image_with_google(prompt, save_path="generated_images.png"):
...
The third part is the ingredients needed, separated by commas (,) (no spaces between commas, but put spaces in the ingredient names as usual). Do not include ingredient numbers. Extra ingredients should fit within the budget. Make sure you use measurements.
The fourth part is the procedures (multiple) to make the dish, separated by periods (.) Do not include procedure numbers.
The fifth part is a description of what the final dish looks like (including its name) that you would give to an image generator to generate an image.
...
PLEASE MAKE SURE YOU FOLLOW THESE CRITERIA. THIS INFORMATION IS CRUCIAL FOR OUR APPLICATION."""
    ...
    generate_image_with_google(answer[4], image_path)
